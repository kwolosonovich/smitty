...
def homepage():
    ...
    formatted_images = search(search_terms=None, dev=False,
...
def search_results(user_id):
    ...
    formatted_images = search(
        search_terms=keyword, max_results=12, dev=False, images_per_row=6, max_rows=2)
...
def add_like(user_id):
    ...
    image = Image.add_image(url, title, artist, date, collection)
...
@app.route('/api/<like_id>/unlike', methods=["POST"])
def unlike(like_id):
    '''Remove liked image.'''
